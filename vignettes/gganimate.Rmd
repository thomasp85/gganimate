...
This means that the final number of frames and its framerate are only ever given
...
function that takes a vector of paths to image files along with a framerate, 
will do. The return value of your renderer is what is ultimately returned by the
`animate()` function.

Below are a couple of examples of different renderers:
