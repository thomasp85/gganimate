...
let it transition between the number of gears available in the cars. As this is
a discrete split (`gear` being best described as an ordered factor) we use 
`transition_states` and provide a relative length to use for transition and 
state view. As not all combinations of data are present, there are states missing
a box. We define that when a box appears it should fade into view, whereas it 
