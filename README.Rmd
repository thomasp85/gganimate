...
## Installation
`gganimate` is currently GitHub-only, but will be released on CRAN once ready. 
In the meantime, install it using the `devtools` package
...
`gganimate` mimicks the way `ggplot2` renders its output, in that the rendering
...
called implicetly as part of `print` the `gif_image` object is available using 
...
the default is to use [gifski](https://github.com/r-rust/gifski) to combines the
