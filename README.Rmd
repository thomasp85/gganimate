...
The `alpha_decay` aesthetic allows you to scale the alpha of older frames through a function 
that takes the difference between the current frame and the frame the data is from and return 
a scalar between 0 and 1.

```{r}
# Get more transparent exponentially
p6 <- ggplot(gapminder, aes(gdpPercap, lifeExp, frame = as.factor(year))) +
  geom_path(aes(cumulative = TRUE, group = country, 
...
```{r}
# Keep the last 3 frames only
p7 <- ggplot(gapminder, aes(gdpPercap, lifeExp, frame = as.factor(year))) +
  geom_path(aes(cumulative = TRUE, group = country, 
                alpha_decay = function(x) x <= 3)) +
  scale_x_log10() +
  facet_wrap(~continent)
